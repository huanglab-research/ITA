...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # learnable query p.e.
        self.query_embed = nn.Embedding(num_queries_pos, hidden_dim)
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        ret["num_queries_pos"] = cfg.MODEL.MASK_FORMER.NUM_OBJECT_QUERIES + 20
    ...
    def forward(self, x, add_tgt, mask_features, mask = None, text_classifier=None, num_templates=None):
        ...
        add_tgt = torch.transpose(add_tgt, 0, 1)  
        query_embed = self.query_embed.weight.unsqueeze(1).repeat(1, bs, 1)
        output = self.query_feat.weight.unsqueeze(1).repeat(1, bs, 1)
        output = torch.cat((add_tgt, output), dim=0)
        predictions_class = []
        predictions_mask = []
