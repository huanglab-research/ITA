...
class FCCLIPHead(nn.Module):
    ...
    def forward(self, features, add_tgt, mask=None):
        return self.layers(features, add_tgt, mask)

    def layers(self, features, add_tgt, mask=None):
        mask_features, transformer_encoder_features, multi_scale_features = self.pixel_decoder.forward_features(features)
        if self.transformer_in_feature == "multi_scale_pixel_decoder":
            predictions = self.predictor(multi_scale_features, add_tgt,mask_features, mask,
